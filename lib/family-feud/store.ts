...
interface GameStore {
  ...
  // Which round `questions` currently holds, so a remount/refresh doesn't
  // blow away in-progress reveals by reloading the same round from scratch.
  questionsLoadedForRound: number | null;

  // Teams (names/scores shown on UI)
  teams: [Team, Team];

  // Core actions
  loadQuestions: (q: Question[], round?: number) => void;
  nextQuestion: () => void;
  prevQuestion: () => void;
...
export const useGameStore = create<GameStore>()(
  persist(
    (set, get) => ({
      ...
      loadQuestions: (q, round) =>
        set({
          questions: q,
          currentIndex: 0,
          current: q[0] ?? null,
          questionsLoadedForRound: round ?? null,
        }),
...
syncStoreAcrossTabs<GameStore>("feud-state", useGameStore.setState);
