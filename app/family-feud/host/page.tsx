...
import { fetchQuestions } from "@/lib/family-feud/data";
import { useGameStore } from "@/lib/family-feud/store";
import * as a from "@/styles/shared/atoms.css";
import { playSound, sound } from "@/lib/shared/sounds";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // If this round's questions are already loaded (e.g. the page was
        // just refreshed), keep the in-progress reveals/index instead of
        // wiping the board back to question 1.
        const already =
          useGameStore.getState().questionsLoadedForRound === currentRound;
        if (already) return;
        ...
        loadQuestions(withMeta, currentRound);
      } catch (err) {
        console.error(
        ...
        if (useGameStore.getState().questionsLoadedForRound !== currentRound) {
          loadQuestions([]);
        }
      }
    })();
    ...
    function onKey(e: KeyboardEvent) {
      ...
      switch (e.key) {
        ...
        case "T": {
          ...
          // but keep already-revealed answers visible on the board —
          // hiding them would wipe out the board every time control swaps.
          clearOrLockPot({ mode: "lock" });
          break;
        }
      }
    ...
    <div className={a.container}>
      ...
      <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
        {(current.answers ?? []).map((ans, i) => (
          ...
          >
            ...
            <button
              onClick={() => {
                const wasRevealed = ans.revealed;
                toggleReveal(i);
                if (!wasRevealed) sound.play("ding");
              }}
              className={a.button({
      ...
      >
        ...
        <button
          onClick={() => {
            sound.play("ding");
            router.push("/family-feud/end-game");
          }}
          className={a.button({ variant: "secondary", size: "lg" })}
        >
          End Game →
