...
/** Build a fresh random particle layout. Call this from an event handler
 * (not during render) and pass the result down as a prop — that way the
 * layout is randomized once per burst instead of reshuffling mid-flight
 * every time the page re-renders while the burst is showing. */
...
function ConfettiBurst({ particles }: { particles: ConfettiParticle[] | null }) {
  const show = !!particles;
  return (
    <AnimatePresence>
      {show && (
        <motion.div
...
export default function SetupPage() {
  ...
  const handleStartFaceoff = () => {
    ...
    if (!readyRef.current) {
      ...
      setBurstParticles(makeConfettiParticles());
      setTimeout(() => {
        setBurstParticles(null);
        router.push("/family-feud/faceoff");
      }, 1000);
    }
  };
  ...
  return (
    <main className={a.container} style={{ position: "relative" }}>
      ...
      <div className={a.buttonsRow} style={{ marginTop: 12 }}>
        ...
        <button
          onClick={() => router.push("/family-feud/game")}
      ...
      <section className={a.buttonsRow} style={{ marginTop: 20 }}>
        <button
          onClick={() => router.push("/family-feud/host")}
        ...
        <button
          onClick={() => router.push("/family-feud/questions")}
      ...
      <ConfettiBurst particles={burstParticles} />
      {Toast}
    </main>
  );
}
